...
# ------------------------------------------------------------------------------
'''
PatientClient class interfaces with Sawtooth through the REST API.
...
FAMILY_NAME = 'patient'
# TF Prefix is first 6 characters of SHA-512("patient"),

def _hash(data):
    return hashlib.sha512(data).hexdigest()

class PatientClient(object):
    '''Client Patient class

    Supports "register", "eat", and "show" functions.
    '''

    def __init__(self, base_url, arguments, key_file=None):
    ...
    def _get_address(self, uid, psid):
        return self._get_prefix() + \
            _hash(str(uid).encode('utf-8'))[0:32] + _hash(str(psid).encode('utf-8'))[0:32]

    # For each CLI command, add a method to:
    # 1. Do any additional handling, if required
    # 2. Create a transaction and a batch
    # 2. Send to REST API
    def register(self, uid, psid, status):
        '''Register user id, ps id, and status for the patient.'''
        return self._wrap_and_send("register", uid, psid, status, wait=10)

    def delete(self, uid, psid):
        '''delete a registered data with a specific user id and ps id.'''
        return self._wrap_and_send("delete", uid, psid, None, wait=10)

    def show(self, uid, psid):
        '''Show the info of a transaction.'''
        address = self._get_address(uid, psid)
    ...
    def _send_to_rest_api(self, suffix, data=None, content_type=None, uid=None, psid=None):
        '''Send a REST command to the Validator via the REST API.

           Called by show() &  _wrap_and_send().
           The latter caller is made on the behalf of register() & delete().
        '''
    ...
    def _wrap_and_send(self, action, uid, psid, status, wait=None):
        '''Create a transaction, then wrap it in a batch.

           Even single transactions must be wrapped into a batch.
           Called by register() and delete().
        '''

        # Generate a CSV UTF-8 encoded string as the payload.
        if action == "register":
            raw_payload = ",".join([action, str(uid), str(psid), status])
        elif action == "delete":
            raw_payload = ",".join([action, str(uid), str(psid)])    
