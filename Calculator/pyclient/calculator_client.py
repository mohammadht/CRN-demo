...
# ------------------------------------------------------------------------------
'''
CalculatorClient class interfaces with Sawtooth through the REST API.
...
# TF Prefix is first 6 characters of SHA-512("calculator"), f7465a

def _hash(data):
    return hashlib.sha512(data).hexdigest()

class CalculatorClient(object):
    '''Client Calculator class

    Supports "add", "sub", and "show" functions.
    '''
    ...
    def __init__(self, base_url, key_file=None):
        ...
        # Address is 6-char TF prefix + hash of "mycalculator"'s public key
    ...
    # 2. Send to REST API
    def add(self, amount):
        '''add amount to the current state'''
        return self._wrap_and_send("add", amount, wait=10)

    def mul(self, amount):
        '''multiply amount by the current state'''
        return self._wrap_and_send("mul", amount, wait=10)

    def sub(self, amount):
        '''subtract amount from the current state'''
        try:
            ret_amount = self._wrap_and_send("sub", amount, wait=10)
        except Exception:
            raise Exception('Encountered an error during subtraction')
        return ret_amount

    def div(self, amount):
        '''divide the current state by amount'''
        return self._wrap_and_send("div", amount, wait=10)
    ...
    def _send_to_rest_api(self, suffix, data=None, content_type=None):
        '''Send a REST command to the Validator via the REST API.
           ...
           The latter caller is made on the behalf of add() & sub().
        '''
